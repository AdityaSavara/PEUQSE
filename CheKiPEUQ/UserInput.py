...
parameter_estimation_settings['multistart_permutationsToSamples'] = True #if this is set to True, then when 'getLogP' is selected then the sampling results will be converted into a statistical sampling distribution so that posterior distribution plots and statistics can be generated. It is presently only for use with gridsearch or uniform distribution search. It will be ignored for other multistart searches.
parameter_estimation_settings['multistart_multistart_permutationsToSamples_threshold_filter_samples'] = True #This setting does nothing. Currently the filtering is always on. This feature removes low probability tails from the posterior. This can be important for getting mu_AP. Default is true. This only has an effect if multistart_permutationsToSamples is set to True.
