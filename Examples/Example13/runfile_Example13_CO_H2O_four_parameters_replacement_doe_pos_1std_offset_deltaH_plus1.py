...
if __name__ == "__main__":
    ...
    UserInput.model['parameterNamesAndMathTypeExpressionsDict'] = {'delta_H_rxn_plus1':'delta_H_rxn_plus1', 'delta_S_rxn':'delta_S_rxn'}
    ...
    delta_H_rxn_plus1 = -0.687 - (-0.858) + 1
    delta_S_rxn = -1.50e-3 - (-1.45E-3)
    delta_H_rxn_plus1_uncertainty = (5.00e-2**2 + 1.25e-1**2)**0.5
    delta_S_rxn_uncertainty = (2.07e-4**2 + 2.07e-4**2)**0.5
    UserInput.model['InputParameterPriorValues'] = [delta_H_rxn_plus1, delta_S_rxn]                                                                               
    UserInput.model['InputParametersPriorValuesUncertainties'] = [delta_H_rxn_plus1_uncertainty, delta_S_rxn_uncertainty] #If user wants to use a prior with covariance, then this must be a 2D array/ list. To assume no covariance, a 1D  This is a standard deviation!
#    UserInput.model['InputParameterInitialGuess'] = [-0.687, 1.50e-3, -0.858, 1.50e-3] #This is where the mcmc chain will start.
    ...
    UserInput.parameter_estimation_settings['scaling_uncertainties_type'] = 1E2
    ...
    temperatures = np.linspace(398.15,498.15,5)
...
#    fig.savefig('synthetic_observables_theta_A.png',dpi=220)
    ...
    prior = np.random.normal(delta_H_rxn_plus1, delta_H_rxn_plus1_uncertainty,200000) #Because we are looking at prior of deltaHrxn which is not temperature dependent, we can do it out here with a high sampling.
    ...
    for p in pressures:
        for t in temperatures:
            fun.pA = p
            fun.T = t
            #Scaling was turned on to try to ensure good sampling of DeltaHRxn. 
            ...
            theta_A_obs_synth = fun.Langmuir_replacement_three_temperatures(ParameterValues_synth) 
