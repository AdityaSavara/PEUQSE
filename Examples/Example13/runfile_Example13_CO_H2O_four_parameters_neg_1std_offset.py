...
if __name__ == "__main__":
    ...
    UserInput.responses['responses_observed_uncertainties'] = 4.34
    ...
    UserInput.model['offset'] = -5.00e-2
    UserInput.model['InputParameterPriorValues'] = [-0.687, 1.50e-3, -0.858, 1.50e-3] # [H_CO, S_CO, H_H2O, S_H2O] eV
    UserInput.model['InputParametersPriorValuesUncertainties'] = [5.00e-2, 2.07e-4, 1.25e-1, 2.07e-4] #If user wants to use a prior with covariance, then this must be a 2D array/ list. To assume no covariance, a 1D  This is a standard deviation!
    ...
    for p in pressures:
        for t in temperatures:
            ...
            prior = np.random.normal(-0.687, 5.00e-2,10000)
            theta_A_obs = fun.Langmuir_compete_ads(parameter_set) 
