...
#    fig.savefig('synthetic_observables_theta_A.png',dpi=220)
    ...
    pos_or_neg = 'pos' #Put 'pos' or 'neg' here.
    loadFromPickleName = pos_or_neg
    loadFromPickle = True
    
    prior = np.random.normal(delta_H_rxn, delta_H_rxn_uncertainty,20000)                                                                                                                                        
    info_gains=[]
    info_gains_KL =[]
    PE_object_list = []
    if loadFromPickle == True:
        import dill
        PE_object_list = dill.load(open("PE_object_list_dill_"+loadFromPickleName+".dill", 'rb'))
        info_gains = dill.load(open("info_gains_list_dill_"+loadFromPickleName+".dill", 'rb'))
    counter = 0
    for p in pressures:
        for t in temperatures:
            fun.pA = p
    ...
    if loadFromPickle == False:
        try:
            import dill
            dillPickleFile = open("PE_object_list_dill_"+loadFromPickleName+".dill", 'wb')
            ...
            dillPickleFile = open("info_gains_list_dill_"+loadFromPickleName+".dill", 'wb')
            ...
            dillPickleFile = open("info_gains"+loadFromPickleName+".dill", 'wb')
            dillPickleFile.close() #It is important to remember to close! Otherwise there can be problems when trying to open later.
