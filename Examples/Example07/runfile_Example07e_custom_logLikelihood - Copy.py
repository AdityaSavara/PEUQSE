...
if __name__ == "__main__":    
    ...
    #Normally, the custom_logLikelihood will point to a function inside another name space, like in processing_function_two_response.
    #However, for this simple example I am makig a function in the run file.
    def doubling_variance_custom_logLikelihood(parameterVector):
        from scipy.stats import multivariate_normal
        #Below we will flatten everything since the multivariate normal command can't take 2D responses and covmat would have to be square.
        simulatedResponses = np.array(UserInput.model['simulateByInputParametersOnlyFunction'](parameterVector)).flatten()
        observedResponses = UserInput.responses['responses_observed'].flatten()
        comprehensive_responses_covmat = ((UserInput.responses['responses_observed_uncertainties']**2)*2).flatten() #square to make variance, then double.
        logLikelihood = multivariate_normal.logpdf(mean=simulatedResponses,x=observedResponses,cov=comprehensive_responses_covmat)
        if sum(simulatedResponses) > 6:
            logLikelihood = float('-inf')
        return logLikelihood, simulatedResponses
        
    UserInput.model['custom_logLikelihood'] = doubling_variance_custom_logLikelihood
