...
UserInput.responses['responses_abscissa'] = np.array([[0,1], [0,1]]) # These represent theta_1 and theta_2 abscissay points. Note that this is *nested* it is a multi-dimensional response, and produces two posteriors.
UserInput.responses['responses_observed'] = np.array([[1,2], [3,4]]) # Note that this is *nested* it is a multi-dimensional response, and produces two posteriors.
UserInput.responses['responses_observed_uncertainties'] = np.array([[1,1], [1,1]])
UserInput.responses['responses_observed_weighting'] = [[1,1], [2,1]] #can change the third to 2 or 0 to see how things change.

UserInput.model['parameterNamesAndMathTypeExpressionsDict'] = {'theta_1':r'$\theta_{1}$','theta_2':r'$\theta_{2}$', 'theta_3':r'$\theta_{3}$','theta_4':r'$\theta_{4}$' }
UserInput.model['InputParameterPriorValues'] = [1,1,5,5 ]
UserInput.model['InputParametersPriorValuesUncertainties'] = [1,1,1,1] #If user wants to use a prior with covariance, then this must be a 2D array/ list. To assume no covariance, a 1D
UserInput.model['InputParameterInitialGuess'] = [1,1,5, 5] #This is where the mcmc chain will start.

UserInput.model['simulateByInputParametersOnlyFunction'] = processing_function_two_response.split_to_two_separated_lists #This must simulate with *only* the parameters listed above, and no other arguments.
...
UserInput.contour_plot_settings['parameter_pairs'] = [[0, 1], [2,3]]
