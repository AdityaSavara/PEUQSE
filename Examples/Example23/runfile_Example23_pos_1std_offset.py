...
if __name__ == "__main__":
    ...
    import processing_function_mem_reactor_three_params as fun
    ...
    UserInput.model['parameterNamesAndMathTypeExpressionsDict'] = {'k_1':r'$k_1 (s^{-1})$', 'k_minus_1':r'$k_{-1} (s^{-1})$', 'k_m':r'$k_m (s^{-1})$'}
    UserInput.model['InputParameterPriorValues'] = [1e2, 1, 2e3] # eV
    UserInput.model['InputParametersPriorValuesUncertainties'] = [5e1, 5e-1, 1e3] #If user wants to use a prior with covariance, then this must be a 2D array/ list. To assume no covariance, a 1D  This is a standard deviation!
    UserInput.model['InputParameterInitialGuess'] = [1e2, 1, 2e3] #This is where the mcmc chain will start.
    ...
    UserInput.parameter_estimation_settings['scaling_uncertainties_type'] = "2e3"
    ...
    UserInput.parameter_estimation_settings['mcmc_burn_in'] = 500 #normally 1000 
    UserInput.parameter_estimation_settings['mcmc_length'] = 8000 #normally 10000
    ...
    UserInput.parameter_estimation_settings['mcmc_info_gain_returned'] = 'KL_divergence'
    
    UserInput.contour_plot_settings['parameter_pairs'] = [[0, 1], [1, 2], [0, 2]]
    ...
    k_1 = 1e2
    k_minus_1 = 1
    ############ Here is the positive 1std offset.
    k_m = 3e3
    ############
    F0 = np.array([5, 0])
    ...
    for v in volumes:
        for t in temperatures:
            ...
            UserInput.contour_plot_settings['figure_name'] = 'prior_and_posterior_contour_T_{}_V_{}'.format(str(t),str(v))
            ...
            fig, ax = plt.subplots()
            ax = PE_object_list[-1].createMumpcePlots()
