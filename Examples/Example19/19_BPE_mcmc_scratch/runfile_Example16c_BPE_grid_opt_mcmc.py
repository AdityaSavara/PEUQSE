...
if __name__ == "__main__":
    ...
    UserInput.model['InputParameterInitialGuess'] = [1.30780046e+00,2.38753805e-03,3.96167260e-01,4.04366288e+01,2.82381353e+01,2.60829373e+01,1.53386412e+01,9.88872337e-08,9.63298973e-02]
...
#    UserInput.model['InputParameterPriorValues_lowerBounds'] = [ 0, None, 0, 0, 0, None, None, 0, 0]
    ...
    UserInput.parameter_estimation_settings['mcmc_burn_in'] = 10#Normally use 100 for example 3a
    UserInput.parameter_estimation_settings['mcmc_length'] = 500#Normally use 10000 for example 3a.
    UserInput.parameter_estimation_settings['mcmc_relative_step_length'] = 0.05
    UserInput.parameter_estimation_settings['mcmc_modulate_accept_probability']  = 10000 #Default value of 0. Changing this value sharpens or flattens the posterior. A value greater than 1 flattens the posterior by accepting low values more often. It can be useful when greater sampling is more important than accuracy. One way of using this feature is to try with a value of 0, then with the value equal to the number of priors for comparison, and then to gradually decrease this number as low as is useful (to minimize distortion of the result). A downside of changing changing this variable to greater than 1 is that it slows the the ascent to the maximum of the prior, so there is a balance in using it. In contrast, numbers increasingly less than one (such as 0.90 or 0.10) will speed up the ascent to the maximum of the posterior, but will also result in fewer points being retained.
    ...
    #PE_object.doGridSearch('doMetropolisHastings', gridSamplingAbsoluteIntervalSize=[ 0.10, 0.005, 0.50/3, 20, 20, 2, 2, 0.1, 0.1], gridSamplingNumOfIntervals=[0,0,1,1,1,1,1,0,0], passThroughArgs={"method":"Nelder-Mead", "maxiter":5000, "verbose":False})#, "maxiter":1000, "verbose":False})
    ...
    #UserInput.parameter_pairs_for_contour_plots = [[3,5],[4,6]]
