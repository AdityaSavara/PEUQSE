...
def tprequation(tpr_theta,t,Ea_1, Ea_2, log_A1, log_A2, gamma1, gamma2,beta_dTdt,start_T): #beta_dTdT is the heating rate. 
    ...
    tpr_theta2D_sum = np.sum(tpr_theta2D, axis=1)
    Ea_Array = [Ea_1,Ea_2] 
    ...
    for rateIndex in range(num_of_concentrations): 
        rate = -tpr_theta2D[:,rateIndex]*np.exp(-(Ea_Array[rateIndex]-kB*T*log_A_array[rateIndex]-gamma_array[rateIndex]*tpr_theta2D_sum)/(kB*T))  
