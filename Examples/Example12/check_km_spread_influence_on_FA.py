...
if __name__ == "__main__":
    ...
    UserInput.responses['responses_observed_uncertainties'] = 0.01
    ...
    UserInput.model['InputParameterPriorValues'] = [1e-3] # eV
    UserInput.model['InputParametersPriorValuesUncertainties'] = [1e-4] #If user wants to use a prior with covariance, then this must be a 2D array/ list. To assume no covariance, a 1D  This is a standard deviation!
    UserInput.model['InputParameterInitialGuess'] = [1e-3] #This is where the mcmc chain will start.
    ...
    UserInput.parameter_estimation_settings['mcmc_relative_step_length'] = 0.0005
    ...
    list_of_km = [1e3, 2e3, 3e3]
    flow_rates_a=[]
    for v in volumes:
        for t in temperatures:
            for km in list_of_km:
                sol = odeint(fun.cmr, F0, np.linspace(0,v,50), args=(k_1,k_minus_1,km,t))
...
'''
    ...
    fun.k_1 = 1e2
    fun.k_minus_1 = 1
    prior = np.random.normal(1e-3,1e-4,10000)
    info_gains=[]
    PE_object_list = []
    for v in volumes:
        for t in temperatures:
