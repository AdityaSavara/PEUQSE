...
responses['reducedResponseSpace'] = [] #If there are multiple responses, the user can ignore some of the responses during parameter estimation. When this feature is on, doSinglePoint will still take the full set of parameters. However, getLogP should not be called directly when this feature is on (as it would require receiving a reduced parmeter vector).
